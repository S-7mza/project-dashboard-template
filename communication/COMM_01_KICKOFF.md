# Communication Plan – Workshop 1: Kickoff

## 1. Objectives
- Announce the project and create alignment.
- Confirm scope, roles, and high-level timeline.
- Capture initial risks, assumptions, and dependencies.

## 2. Stakeholder Groups
- **Primary:** Sponsor, Product Owner, Tech leads.
- **Secondary:** Key business stakeholders, BI/IT management.

## 3. Timeline & Touchpoints
- **T–5 to T–3 days:** Save-the-date + context.
- **T–2 days:** Detailed agenda + pre-work reminder.
- **T–0 morning:** "Starting today" reminder.
- **T+0 afternoon:** Thank you + minutes + action items.
- **T+2 days:** Follow-up on critical actions.

---

## 4. Pre-Workshop Email – Save the Date (T–5 to T–3)

**From:** Tech Manager
**To:** Sponsor; Product Owner; Tech leads; key stakeholders
**Subject:** `[Dashboard Project] Kickoff – hold the date`

Hi all,

We are launching the **Dashboard Project** and would like to hold a **Kickoff workshop** to align on objectives, scope, roles, and the high-level plan.

**Proposed slot:** Week 0, Day 0 – 60–90 minutes
**Participants:** Sponsor, Product Owner, Tech leads

Please keep this time blocked in your calendars. A detailed agenda and pre-work will follow.

Best regards,
Tech Manager


## 5. Pre-Workshop Email – Agenda & Pre-work (T–2)

**From:** Tech Manager
**To:** Confirmed participants
**Subject:** `[Dashboard Project] Kickoff – agenda & pre-work`

Hi all,

Thank you for holding time for the **Dashboard Project Kickoff**. Below are the details:

**When:** Week 0, Day 0 – 60–90 minutes  
**Where:** <meeting room or virtual link>  
**Participants:** Sponsor; Product Owner; Tech leads

**Agenda (draft)**
1. Welcome & introductions (5 min)
2. Business objectives & success criteria (15 min)
3. Scope, constraints, and assumptions (20 min)
4. Roles & RACI overview (10 min)
5. High-level roadmap & key milestones (15 min)
6. Risks, dependencies, and next steps (15–25 min)

**Pre-work (due before the meeting)**

Please let me know if you have any additions or constraints for the agenda.

Best regards,
Tech Manager


## 6. Same-Day Reminder (T–0 morning)

**From:** Tech Manager  
**To:** Participants  
**Subject:** `Reminder – Dashboard Project Kickoff today`

Hi all,

A quick reminder that our **Dashboard Project Kickoff** is today:


Please bring:

See you there,
Tech Manager


## 7. Post-Workshop Email – Minutes & Actions (T+0 / T+1)

**From:** Tech Manager  
**To:** Participants; key stakeholders (cc Sponsor)
**Subject:** `[Dashboard Project] Kickoff – recap & next steps`

Hi all,

Thank you for your time and contributions during the **Dashboard Project Kickoff**. Below is a brief summary.

**1. Confirmed objectives**

**2. Scope & key decisions**

**3. Risks & dependencies**

**4. Actions** (logged in `templates/ACTION_LOG.csv`)

**5. Next steps**

Minutes are captured in `templates/MEETING_MINUTES.md`, and RACI is initialized in `templates/RACI.md`.

Best regards,
Tech Manager


## 8. Reminders & Follow-up (T+2)

Send a short reminder to owners of critical actions if due dates are near or overdue, referencing the **Action Log** and linking to the next workshop invite when applicable.
